...
2. SWITCH statement cannot have (dynamic) arrays declared in any one of the cases because that array might be
    used in some other case (and the one with declaration might never run in the first place). So, we do not
    know how much space to reserve on stack for this array.
...
4. The variable bounds in dynamic arrays are are allowed to be negative as opposed to static arrays for which they are
    always positive (being NUM).